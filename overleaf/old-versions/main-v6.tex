...
\sz{2D complexity to be added, even though maybe mild improvement.}

\sz{Tr and tr to be unified.}

\textcolor{red}{Shuo: Just now I realized that an evidence for sample complexity saving over 2D extrapolation can be that, if we consider the same grid baseline as 1D extrapolation where each point error is allowed to be $\frac{\epsilon}{\norm{b}_1}=\frac{\epsilon}{\log(m)}$, however in 2D that is $\frac{\epsilon}{\norm{b}_1\norm{b'}_1}$, only $\norm{b}_1$ is well-conditioned. $\norm{b'}_1$ is a square/cubic node maybe $\operatorname{poly}(m)$ or even $exp(m)$, causing additional polynomial sampling overhead inversely with error allowance. I have not analyzed the $m$ difference yet.}
...
% If we use this perspective, I think our abstract should contain like \emph{Exponentially Reduced Circuit Depth for Trotterization under Physical Noise with Rigorous (General) Commutator Bounds} \sz{Now this part of contribution has been included in another work.}
...
\begin{theorem}[Sample-complexity comparison with a two-dimensional grid]
\label{thm:sample-complexity-2d-comparison}
\normalfont\color{blue}
Shuo: The following gives a possible way to formalize the sampling advantage over a two-dimensional extrapolation baseline, using the same additive-error accounting as in the proof above. In the coupled one-dimensional protocol, the estimator is
\begin{align*}
    \widehat F_{\rm 1D}=\sum_{j=1}^{m} b_j \widehat f(s_j),
\end{align*}
where the extrapolation nodes are chosen so that $\|\mathbf b\|_1=O(\log m)$. If each grid value satisfies $|\widehat f(s_j)-f(s_j)|\le \delta_{\rm 1D}$, then
\begin{align*}
    |\widehat F_{\rm 1D}-F_{\rm 1D}|
    \le \|\mathbf b\|_1 \delta_{\rm 1D}.
\end{align*}
Thus it is sufficient to choose $\delta_{\rm 1D}=O(\eps\|O\|/\|\mathbf b\|_1)$. Using the same standard observable-estimation cost as in the proof above, this accuracy requires, up to logarithmic confidence factors,
\begin{align*}
    n_{\rm 1D}=O\left(\frac{\|\mathbf b\|_1^2}{\eps^2}\right)
    =O\left(\frac{\log^2 m}{\eps^2}\right),
\end{align*}
repetitions per node. Hence the total number of circuit repetitions is $O(m\|\mathbf b\|_1^2/\eps^2)$, before accounting for the circuit depth associated with each node.

In comparison, consider a tensor-product two-dimensional Richardson rule that extrapolates independently in the step-size variable and in the physical-noise variable:
\begin{align*}
    \widehat F_{\rm 2D}=\sum_{j=1}^{m}\sum_{\ell=1}^{m'} b_j b'_{\ell}\widehat g(s_j,\lambda_\ell).
\end{align*}
If each two-dimensional grid value is estimated to additive error $\delta_{\rm 2D}$, then
\begin{align*}
    |\widehat F_{\rm 2D}-F_{\rm 2D}|
    \le \|\mathbf b\|_1\|\mathbf b'\|_1
    \delta_{\rm 2D}.
\end{align*}
Thus each grid point must be estimated to accuracy $O(\eps\|O\|/(\|\mathbf b\|_1\|\mathbf b'\|_1))$, giving the sample cost
\begin{align*}
    n_{\rm 2D}=O\left(\frac{\|\mathbf b\|_1^2\|\mathbf b'\|_1^2}{\eps^2}\right)
\end{align*}
repetitions per grid point and total number of circuit repetitions
\begin{align*}
    O\left(\frac{mm'\|\mathbf b\|_1^2\|\mathbf b'\|_1^2}{\eps^2}\right).
\end{align*}
Therefore, even if the step-size direction is well-conditioned, the tensor-product two-dimensional estimator pays an additional factor $\|\mathbf b'\|_1^2$ in the repetition count, in addition to the larger number $mm'$ of grid points.

The possible danger is that the second extrapolation direction need not be well-conditioned. For generic nodes $x_\ell$ extrapolated to $x=0$, the Richardson coefficients obey the Lagrange formula
\begin{align*}
    b'_\ell=\prod_{q\ne \ell}\frac{x_q}{x_q-x_\ell},
    \qquad
    \|\mathbf b'\|_1=\sum_{\ell=1}^{m'}\left|
    \prod_{q\ne \ell}\frac{x_q}{x_q-x_\ell}
    \right|.
\end{align*}
Unlike the Chebyshev-type well-conditioned construction, square/cubic or otherwise non-optimized grids can make this norm grow as $\|\mathbf b'\|_1=\exp(\Omega(m'))$. If the two-dimensional baseline needs extrapolation order $m'=\Theta(\log(1/\eps))$ in the noise direction to reduce the deterministic extrapolation bias below $O(\eps)$, then
\begin{align*}
    \|\mathbf b'\|_1^2=\exp(\Omega(m'))=\operatorname{poly}(1/\eps),
\end{align*}
and the 2D estimator suffers an additional polynomial factor in $1/\eps$ in its sampling complexity. This comparison does not require using $m'=\Theta(\log(1/\eps))$ universally; rather, it says that whenever the independent noise-direction extrapolation must be taken to logarithmic order in $1/\eps$ and is not well-conditioned, the 2D tensor-product strategy can lose a polynomial sampling factor relative to the coupled 1D path.
\end{theorem}
...
\end{equation*}
where $w_k=\left(1+e^{-2 \lambda_k}\right) / 2$. \sz{Quantum circuit schematic figure.}
