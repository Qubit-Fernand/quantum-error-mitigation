...
\label{eq:bch}
    ...
    \sum_{\sigma\in S_q} c_{\sigma, q}\mc R_{\sigma}
...
\begin{proof}
    ...
    \begin{align*}
        &|R_m(s)| \\
        \le ~&\sum_{q=m}^{\infty}\|O\|\|\tilde{\E}_{(q_0),q}\|\|\rho_0\|_1 s^q \\
        &+\|O\|\Big\|(\tilde{\mc P}(sT, c(sT)^{p+1}))^{1/s}
        -(e^{\G_{(q_0)}(s)sT})^{1/s}\Big\|\|\rho_0\|_1 \\
        \le ~& \sum_{q=m}^{\infty} e(s(2\mu_{\mathrm{comm},q_0} T)^{1+1/p})^{q}\|O\|\\
        &+\frac{(\max\{1, \|e^{\G_{(q_0)}(s) sT}\|\})^{1/s}}{s}
        \Bigl\|\tilde{\mc P}(sT, c(sT)^{p+1})-e^{\G_{(q_0)}(s)sT}\Bigr\|\|O\|\\
        \le ~&2e^{1-m} \|O\| \\
        &+\frac{(1+e\alpha_{\mathrm{comm},q_0}(s))^{1/s}}{s}
...
$\mb E X_{i,\ell}=\mu_i$, $\mathrm{Var}(X_{i,\ell})=\sigma_i^2$, and
$|X_{i,\ell}|\le \|O\|$.
Then
\begin{align}
...
\label{eq:equal-shot-bound}
    ...
    \begin{aligned}
    &\left(
    \frac{K\sum_{i=1}^K a_i^2\sigma_i^2}{\eta^2}\right.\\
    &\left.\qquad+
...
\label{eq:optimized-shot-bound}
    ...
    \begin{aligned}
    &\left(
    \frac{\left(\sum_{i=1}^{K}|a_i|\sigma_i\right)^2}{\eta^2}\right.\\
    &\left.\qquad+
...
\label{eq:1d-equal-shot}
    ...
    \begin{aligned}
    &\left(
    \frac{m\sum_j b_j^2\sigma_{j,\mathrm{path}}^2}{\eta^2}\right.\\
    &\left.\qquad+
...
\label{eq:1d-opt-shot}
    ...
    \begin{aligned}
    &\left(
    \frac{\left(\sum_j |b_j|\sigma_{j,\mathrm{path}}\right)^2}{\eta^2}\right.\\
    &\left.\qquad+
...
\label{lem:power-bound}
    ...
    \end{align*}
\end{lemma}
\section{BCH truncation error of different Hamiltonians}
\label{sec:bch-truncation-apps}
In this section, we discuss how the previous lemma applies to different Hamiltonians, including local and second quantization.
...
The hardware experiment uses a connected 9-qubit subgraph of a 13-qubit superconducting processor.
Before learning the effective gate-noise channel, we mitigate readout error at the observable level.
Figure~\ref{fig:app-readout} shows both the full confusion matrix on the 9-qubit subgraph and the
local single-qubit calibration matrices. The global matrix is strongly diagonally dominant, and the
local matrices capture the dominant contribution for the magnetization observable.
...
\begin{figure}[!htbp]
    ...
    (a) Full $2^9\times2^9$ global confusion matrix.
    (b) Single-qubit local calibration matrices used for observable-level readout mitigation.
    }
    \label{fig:app-readout}
\end{figure}

\subsection{Noise Channel Learning}
\label{app:noise-learning}
...
\]
where $\mathcal{K}$ contains only the Pauli components retained by the sparse model.
Training circuits are produced by replacing the target circuit rotation angles by Clifford angles in $\{0,\pi/2,\pi,3\pi/2\}$ while preserving the same circuit layout, qubit mapping and layer structure. Solving the resulting sparse inverse problem yields the effective Pauli--Lindblad rates.

Figure~\ref{fig:app-learned-noise} shows a representative learned hardware noise channel for a Trotterized circuit with step number $N=3$.
...
Figure~\ref{fig:app-workflow} illustrates the overall experimental learning-and-mitigation workflow. Target Trotter circuits are mapped to the hardware topology and compiled into native gates. Cliffordized training circuits preserve the same layer structure while replacing rotation angles by Clifford angles. Hardware measurements of these training circuits are used to learn an effective layerwise sparse Pauli--Lindblad noise model, which enables controlled noise amplification for ZNE.
...
\begin{figure}[!htbp]
    ...
    Hardware measurements of these training circuits are used to learn an effective layerwise sparse Pauli--Lindblad noise model, which enables controlled noise amplification for ZNE.
    }
    \label{fig:app-workflow}
\end{figure}

After learning the effective noise channel, we use random Pauli insertions to implement gain factors for the noise strength. Figure~\ref{fig:app-zne} shows hardware ZNE data for two representative evolution settings.
...
The hardware ZNE stage removes only the physical-noise bias at fixed Trotter step number. The remaining deterministic product-formula error is then extrapolated in the Trotter step size, as described in the main text.
